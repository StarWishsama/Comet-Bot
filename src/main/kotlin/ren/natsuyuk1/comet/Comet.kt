...
import ren.natsuyuk1.comet.CometVariables.logger
...
class Comet {
    ...
    var id: Long = 0
        set(value) {
            ren.natsuyuk1.comet.CometVariables.cfg.botId = value
            field = value
        }
    var password: String = ""
        set(value) {
            ren.natsuyuk1.comet.CometVariables.cfg.botPassword = value
            field = value
        }
    ...
    suspend fun login() {
        ...
        ren.natsuyuk1.comet.CometVariables.daemonLogger.info("正在配置登录配置...")
        ...
        try {
            ...
            ren.natsuyuk1.comet.CometVariables.daemonLogger.warning("登录失败! 返回的失败信息: ${e.message}")
    ...
    private fun initBot() {
        val config = BotConfiguration.Default.apply {
            botLoggerSupplier = { it ->
                ren.natsuyuk1.comet.CustomLogRedirecter(
                    "Mirai (${it.id})",
                    ren.natsuyuk1.comet.CometVariables.miraiLogger
                )
            }
            networkLoggerSupplier = { it ->
                ren.natsuyuk1.comet.CustomLogRedirecter(
                    "MiraiNet (${it.id})",
                    ren.natsuyuk1.comet.CometVariables.miraiNetLogger
                )
            }

            fileBasedDeviceInfo()

            protocol = ren.natsuyuk1.comet.CometVariables.cfg.botProtocol

            heartbeatStrategy = ren.natsuyuk1.comet.CometVariables.cfg.heartbeatStrategy
        }
