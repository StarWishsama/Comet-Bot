    ...
    override suspend fun execute(event: MessageEvent, args: List<String>, user: CometUser): MessageChain {
        ...
        if (event is GroupMessageEvent) {
            if (args.isNotEmpty()) {
                ...
                when (args[0].lowercase()) {
                        ...
                        return if (args.size == 2) {
                            ...
                            if (target != null) {
                                if (cfg.isHelper(target.id)) {
                                    ...
                                    toMessageChain("成功将 ${target.id} 移出群助手列表")
                                } else {
                                    cfg.addHelper(target.id)
                                    toMessageChain("成功将 ${target.id} 加入群助手列表")
                                }
                            } else {
                    ...
                    "repeat" -> {
                        ...
                        return toMessageChain("已${if (cfg.canRepeat) "开启" else "关闭"}群复读机")
                    }
                    "autojoin" -> {
                        if (args.size > 1) {
                            return when (args[1]) {
                            ...
                            return if (event.group.botPermission == MemberPermission.MEMBER) {
                                ...
                                "已${if (cfg.autoAccept) "开启" else "关闭"}自动接受加群请求".toMessageChain()
                            }
                        }
                    }
                    "function", "fun", "func" -> {
            ...
            """.trimIndent()
                        ...
                        return cmd?.props?.disableCommand(event.group.id)?.msg?.toMessageChain() ?: "找不到对应命令".toMessageChain()
                    }
                        ...
                        if (args.size > 1) {
                            when (args[1]) {
                                "switch" -> {
                                    ...
                                    return "自动删除文件功能: ${cfg.oldFileCleanFeature}".toMessageChain()
                                }
                                "delay" -> {
                                    if (args.size > 2) {
                                        val delay = args[2].toLongOrNull() ?: return "请输入正确的数字".toMessageChain()
                                        cfg.oldFileCleanDelay = delay
                                        return "已设置自动删除超过 $delay ms 的文件.".toMessageChain()
                                    } else {
                                        return "/group frm delay [删除延迟]".toMessageChain()
                                    }
                                }
                                "pattern" -> {
                                    return if (args.size > 2) {
                                        cfg.oldFileMatchPattern = args[2]
                                        "已设置文件匹配正则表达式为 [${cfg.oldFileMatchPattern}].".toMessageChain()
                                    } else {
            ...
            return "抱歉, 该命令仅供群聊使用!".toMessageChain()
        }
    }
