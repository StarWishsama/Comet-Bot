        ...
        when (waitList[user] ?: 0) {
            ...
            1 -> {
                ...
                if (port.isNumeric()) {
                    ...
                    event.subject.sendMessage(CometUtil.sendMessage("设置密码成功!\n请在下一条消息发送 rCon 密码\n" +
                            "如果需要退出设置 请回复退出"))
                    waitList[user] = 2
                } else {
                    event.subject.sendMessage(CometUtil.sendMessage("不是有效的端口\n" +
                            "如果需要退出设置 请回复退出"))
                }
            }
            2 -> {
                BotVariables.cfg.rConPassword = event.message.contentToString()
                event.subject.sendMessage(CometUtil.sendMessage("设置 rCon 完成!"))
                Comet.setupRCon()
                waitList.remove(user)
                SessionManager.expireSession(session)
            }
