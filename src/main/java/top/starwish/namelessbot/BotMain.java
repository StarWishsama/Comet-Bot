...
public class BotMain extends JcqAppAbstract implements ICQVer, IMsg, IRequest {
    ...
    public int privateMsg(int subType, int msgId, long fromQQ, String msg, int font) {
        if (msg.startsWith("/") || msg.startsWith("#")) {
            ...
            if (isBotAdmin || isOwner || fromQQ == 1552409060) {
                switch (cmd[0]) {
                case "say":
                    String message = msg.replaceFirst("/" + cmd[0] + " ", "").replaceFirst(cmd[1] + " ", "")
                            .replaceFirst("#", "");
                    ...
                    if (isOwner) {
                        ...
                        case "add":
                            if (!cmd[2].equals("") && StringUtils.isNumeric(cmd[2])) {
                                ...
                                mySendPrivateMsg(fromQQ, "Bot > 已成功添加管理员 " + CQ.getStrangerInfo(adminQQ).getNick() + "("
                                        + adminQQ + ")");
                            }
                            break;
                        case "del":
                            if (!cmd[2].equals("") && StringUtils.isNumeric(cmd[2])) {
                                ...
                                mySendPrivateMsg(fromQQ, "Bot > 已成功删除管理员 " + CQ.getStrangerInfo(adminQQ).getNick() + "("
                                        + adminQQ + ")");
                                break;
                            }
                        }
        ...
        if (msg.startsWith("/") || msg.startsWith("#")) {
            ...
            if (botStatus) {
                        ...
                        if (cmd[1].equals("")) {
                            ...
                            try {
                                ...
                                if (banTime <= 30 * 24 * 60 * 60) {
                                    ...
                                    mySendGroupMsg(fromGroup, "Bot > 已禁言 " + CQ.getStrangerInfo(banQQ).getNick() + "("
                                            + banQQ + ") " + banTime / 60 + "分钟.");
                                } else
                ...
                case "debug":
                    if (isBotAdmin || isOwner) {
                        ...
                        case "checkin":
                            ...
                            case "set":
                                try {
                                    long userQQ = StringUtils.isNumeric(cmd[3]) ? Integer.parseInt(cmd[3])
                                            : CC.getAt(cmd[3]);
                                    BotUser user = checkinUsers.get(userQQ);
                                    double point = Double.parseDouble(cmd[4]);
                                    user.setCheckInPoint(point);
                                    mySendGroupMsg(fromGroup,
                                            "Bot > 已设置 " + CQ.getStrangerInfo(userQQ).getNick() + " 的积分为 " + point);
                            ...
                            case "reset":
                                try {
                                    long userQQ1 = StringUtils.isNumeric(cmd[3]) ? Integer.parseInt(cmd[3])
                                            : CC.getAt(cmd[3]);
                                    checkinUsers.remove(userQQ1);
                                    mySendGroupMsg(fromGroup,
                                            "Bot > 已重置 " + CQ.getStrangerInfo(userQQ1).getNick() + " 的签到账号");
                                    break;
                                } catch (Exception ignored) {
                                    mySendGroupMsg(fromGroup, "Bot > 请检查你输入的命令!");
                                }
                    ...
                    if (isOwner || isBotAdmin) {
                        ...
                        case "add":
                            if (isOwner) {
                                if (!cmd[2].equals("")) {
                                    try {
                                        long adminQQ = StringUtils.isNumeric(cmd[2]) ? Integer.parseInt(cmd[2])
                                                : CC.getAt(cmd[2]);
                                        adminIds.add(adminQQ);
                                        mySendGroupMsg(fromGroup, "Bot > 已成功添加管理员 "
                                                + CQ.getStrangerInfo(adminQQ).getNick() + "(" + adminQQ + ")");
                                    } catch (Exception e) {
                                        mySendGroupMsg(fromGroup, "Bot > 请检查你输入的命令!");
                                    }
                                }
                                ...
                                if (!cmd[2].equals("")) {
                                    try {
                                        long adminQQ = StringUtils.isNumeric(cmd[2]) ? Integer.parseInt(cmd[2])
                                                : CC.getAt(cmd[2]);
                                        adminIds.remove(adminQQ);
                                        mySendGroupMsg(fromGroup, "Bot > 已成功移除管理员 "
                                                + CQ.getStrangerInfo(adminQQ).getNick() + "(" + adminQQ + ")");
                                    } catch (Exception x) {
                                        mySendGroupMsg(fromGroup, "Bot > 请检查你输入的命令!");
                                    }
                ...
                case "checkin":
                    if (!checkinUsers.containsKey(fromQQ) && cmd[1].equals("")) {
                        ...
                        if (BotUtils.isCheckInReset(new Date(), checkinUsers.get(fromQQ).getLastCheckInTime())) {
                            ...
                            double basePoint = Double.parseDouble(String.format("%.1f",
                                    new Random(Calendar.getInstance().getTimeInMillis()).nextInt(10)));
                            double awardPoint = awardProp < 3 ? awardProp * basePoint : 3 * basePoint;
                ...
                case "shop":
                    ...
                    case "add":
                        if (isOwner || isBotAdmin) {
                            if (!cmd[2].equals("") && StringUtils.isNumeric(cmd[3]) && !cmd[5].equals("")
                                    && StringUtils.isNumeric(cmd[4])) {
                                Shop shop = new Shop();
                                ...
                                mySendGroupMsg(fromGroup,
                                        "Bot > /shop add [商品名] [需要积分] [执行命令] [可购买次数]\n玩家绑定账号可使用 %p%代替!\n可购买次数设为 0 以改为无限购买");
                        } else
                            mySendGroupMsg(fromGroup, "Bot > 你没有权限!");
                        break;
                    case "del":
                                        ...
                                        try {
                                            ...
                                            if (shop.getItemCommand().contains("give")) {
                                                ...
                                                if (!status.contains("离线")) {
                                                    ...
                                                    checkinUsers.get(fromQQ)
                                                            .setCheckInPoint(checkinUsers.get(fromQQ).getCheckInPoint()
                                                                    - shop.getItemPoint());
    ...
    public void readConf() {
        try {
            ...
            JSONObject adminsObject = JSONObject
                    .parseObject(FileProcess.readFile(CQ.getAppDirectory() + "admins.json"));
            ...
            if (JSON.parseObject(groupsObject.getString("groupSetting"), new TypeReference<List<QQGroup>>() {
            }) != null)
                groupSetting = JSON.parseObject(groupsObject.getString("groupSetting"),
                        new TypeReference<List<QQGroup>>() {
                        });

            JSONObject settingObject = JSONObject
                    .parseObject(FileProcess.readFile(CQ.getAppDirectory() + "botsettings.json"));
            filterWords = JSON.parseObject(settingObject.getString("triggerWords"), new TypeReference<List<String>>() {
            });

            JSONObject checkInObject = JSONObject
                    .parseObject(FileProcess.readFile(CQ.getAppDirectory() + "qiandao.json"));
            checkinUsers = JSON.parseObject(checkInObject.getString("checkinUsers"),
                    new TypeReference<Map<Long, BotUser>>() {
                    });
        ...
        if (botStatus) {
            for (QQGroup g : groupSetting) {
                if (g.isAutoAcceptRequest() && subtype == 1) {
                    ...
                    mySendGroupMsg(fromGroup,
                            "Bot > 已自动接受来自 " + CQ.getStrangerInfo(fromQQ).getNick() + "(" + fromQQ + ") 的入群申请.");
                    break;
        ...
        if (botStatus && solidot.getStatus()) {
            ...
            if (!solidotTemp.exists()) {
                ...
                if (!tempTitle.equals("") && !tempTitle.equals(title)
                        && !tempTitle.equals("Encountered a wrong URL or a network error.")
                        && !title.equals("Encountered a wrong URL or a network error.")) {
